...
//Driver funcction - it loops through the table to collect the tokens, line numbers and token types
...
//Check the character. Once Identified, return the respective value to nextChar index
...
//function that removes a single line comments
		...
		if (input[i] == '\\') {
			...
			for(j=i+1; j < len; j++ ){
				...
				if(input[j] == '\\' && input[j] != '\n'){
				 ...
				 else { //otherwise replace the entire line
					input[j] = ' ';
				}
...
void checkIfKeyword() {
	...
	int i,j;
	char *keywords[] = {"Begin","End","Loop","Void","INT","Return","Read","Output","Program","IFF","Then","Let"};
		...
		if(tokens[i].tokenType == IDENTIFIER){
			for (j=0; j < len; j++){
				...
				if(strcmp(tokens[i].instance, "Output") == 0){
					tokens[i].tokenType = OUTPUT_KEYWORD;	
				}
				...
				if(strcmp(tokens[i].instance, "IFF") == 0){
					tokens[i].tokenType = IFF_KEYWORD;	
				}
...
void checkError(){	
	...
	if(tokens[z].tokenType == IDENTIFIER){
			...
			//check if first letter is uppercase and show error if not
